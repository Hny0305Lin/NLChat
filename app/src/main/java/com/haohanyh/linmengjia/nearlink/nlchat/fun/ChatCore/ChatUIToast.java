    ...
    public static void SnackBarToastForDebug(Context context, String text, String actiontext, int level, int SnackbarLength) {
        ...
        switch (level) {
            case 0: // VERBOSE日志 不重要的提示 黑
                snackbar.getView().setBackgroundColor(context.getResources().getColor(R.color.snackbar_log_v));
                break;
            case 1: // DEBUG日志 稍微重要的提示 蓝
                snackbar.getView().setBackgroundColor(context.getResources().getColor(R.color.snackbar_log_d));
                break;
            case 2: // INFO日志 一般重要的提示 绿
                snackbar.getView().setBackgroundColor(context.getResources().getColor(R.color.snackbar_log_i));
                break;
            case 3: // WARN日志 已经算重要的提示 橙
                snackbar.getView().setBackgroundColor(context.getResources().getColor(R.color.snackbar_log_w));
                break;
            case 4: // ERROR日志 很重要的提示 红
                snackbar.getView().setBackgroundColor(context.getResources().getColor(R.color.snackbar_log_e));
                break;
            default: // 随意啦
                snackbar.getView().setBackgroundColor(context.getResources().getColor(R.color.blue_biaozhun_logowai_transparent));
                break;
        }
